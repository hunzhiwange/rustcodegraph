...
pub fn build_windows_upgrade_script(bundle_root: &str, version: &str, arch: &str) -> String {
    ...
    [
        ...
        "$newExe=Join-Path $stage 'rustcodegraph.exe'".to_owned(),
        "if(-not (Test-Path $newExe)){throw 'rustcodegraph: downloaded archive did not contain rustcodegraph.exe'}".to_owned(),
        ...
        "Copy-Item -Path (Join-Path $stage '*') -Destination $dest -Recurse -Force".to_owned(),
        "Copy-Item -Path $newExe -Destination $exe -Force".to_owned(),
        "Remove-Item -Path (Join-Path $dest 'rustcodegraph.exe') -Force -ErrorAction SilentlyContinue".to_owned(),
