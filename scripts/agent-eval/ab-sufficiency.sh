...
#
# WITH rustcodegraph (pre-warmed daemon, reliable nested attach) vs WITHOUT (empty
...
"$BIN" init "$TGT" >/dev/null 2>&1 && echo "indexed copy ($("$BIN" status --json 2>/dev/null | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{try{console.log(JSON.parse(s).fileCount+" files")}catch{console.log("?")}})' 2>/dev/null || echo '?'))"

echo "###### repo=$REPO  runs/arm=$RUNS"
echo "###### Q=$Q"; echo
echo '{"mcpServers":{}}' > "$OUT/mcp-empty.json"
printf '{"mcpServers":{"rustcodegraph":{"command":"%s","args":["serve","--mcp","--path","%s"]}}}' "$BIN" "$TGT" > "$OUT/mcp-cg.json"

prewarm(){
  pkill -9 -f "serve --mcp --path $TGT" 2>/dev/null
  RUSTCODEGRAPH_DAEMON_IDLE_TIMEOUT_MS=1800000 "$BIN" serve --mcp --path "$TGT" </dev/null >/dev/null 2>&1 &
  node -e 'const fs=require("fs");let n=0;const t=setInterval(()=>{if(fs.existsSync(process.argv[1]+"/.rustcodegraph/daemon.sock")){clearInterval(t);process.exit(0)}if(n++>150){clearInterval(t);process.exit(1)}},100)' "$TGT" >/dev/null 2>&1
}
...
echo "== WITH rustcodegraph (premise: explore/node used -> Read ~0) =="; run with "$OUT/mcp-cg.json" 1
