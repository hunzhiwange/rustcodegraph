...
# metric is tool CHOICE: Read calls vs rustcodegraph_node[file] calls per run.
#
...
# Per-run tool-choice counts: Read vs rustcodegraph_node[file] vs [symbol].
count() {
  node -e '
    const fs=require("fs");
    ...
    for(const l of lines){try{const o=JSON.parse(l);
      ...
      for(const b of (Array.isArray(blocks)?blocks:[])){
        ...
        if(b.name==="Read")read++;
        else if(b.name==="mcp__rustcodegraph__rustcodegraph_node"){ if(b.input&&b.input.symbol)cgSym++; else cgFile++; }
        else if(/mcp__rustcodegraph__/.test(b.name))cgOther++;
      }
    }catch{}}
    console.log(`    Read=${read}  rustcodegraph_node[file]=${cgFile}  rustcodegraph_node[symbol]=${cgSym}  other_cg=${cgOther}  (cg exposed=${exposed})`);
  ' "$1"
}
...
echo "== NEW build (HEAD: rustcodegraph_node has Read parity) =="
...
echo "###### DONE — compare [new] vs [baseline]: does rustcodegraph_node[file] rise / Read fall? Logs: $OUT"
