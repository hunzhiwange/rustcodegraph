...
The node and edge kinds the graph is built from.

RustCodeGraph stores three things: **nodes** (symbols and files), **edges** (relationships between them), and **files**. Every node and edge carries an exact `kind`, drawn from a fixed vocabulary so queries are consistent across languages.
...
See the [CLI](../reference/cli.md) and [MCP Server](../reference/mcp-server.md) references for how to run these.
