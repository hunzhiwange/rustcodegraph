...
`rustcodegraph telemetry` CLI, MCP + installer wiring, `TELEMETRY.md`. Pending: Worker deploy
+ DNS, release.
Scope: public RustCodeGraph engine (CLI + MCP server + installer)

RustCodeGraph is a local-first tool whose whole pitch is "your code never leaves your machine."
...
- How many machines actively use RustCodeGraph (daily/weekly), and how does that change?
...
- No telemetry from the `rustcodegraph-pro` fork (see "rustcodegraph-pro rule" below).
...
| `rustcodegraph_version` | `0.9.12` | from package metadata |
...
- **`usage_rollup`** — the workhorse. One event per `(day, kind, name)` per machine,
  ...
  (e.g. `rustcodegraph_explore`, `affected`), `count`, `error_count`, and for MCP:
...
2. `RUSTCODEGRAPH_TELEMETRY=0|1` → forced off/on for that process
3. Global config `~/.rustcodegraph/telemetry.json` → stored user choice
...
- **Headless paths** (`npx rustcodegraph init`, MCP server — no TTY, never prompt): right
...
- **CLI:** `rustcodegraph telemetry status|on|off` (status prints the machine ID, current
  state, and what decided it). Deleting `~/.rustcodegraph/telemetry.json` resets everything,
...
(`~/.rustcodegraph/` is new — today nothing global exists. Coexists by filename if a user ever
indexes `$HOME` itself, since per-project data lives in `<project>/.rustcodegraph/` with fixed
other filenames.)
...
- **Buffer** — counters persist (debounced, async) to `~/.rustcodegraph/telemetry-queue.jsonl`.
...
- **Flush** — many CLI actions end via `process.exit()`, where `beforeExit` never fires
  ...
  sender's claim merges back after an hour). `RUSTCODEGRAPH_TELEMETRY_DEBUG=1` echoes
...
`telemetry.getcodegraph.com` → small Rust/Wasm Worker living at `telemetry-worker/` in this repo —
...
## rustcodegraph-pro rule (do not lose this in upstream merges)

The private `rustcodegraph-pro` fork ships inside customer containers whose guarantee is
...
container sets `RUSTCODEGRAPH_TELEMETRY=0` as belt-and-braces. This rule lives in the fork's
...
2. Worker + DNS live first (so the first shipping client never 404s), PostHog dashboards:
   ...
   languages indexed.
3. Client module + config + `rustcodegraph telemetry` subcommand + MCP `clientInfo` plumbing.
