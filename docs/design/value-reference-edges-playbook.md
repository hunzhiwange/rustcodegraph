...
**Where it flows:** straight into `getImpactRadius` → `rustcodegraph impact` and the impact trail
in `rustcodegraph_explore` / `rustcodegraph_node`. No agent-behaviour change required. **The win is
...
| `valueRefsEnabled` | `process.env.RUSTCODEGRAPH_VALUE_REFS !== '0'` — default ON, env opts out. |
...
- **Default ON** for TS/JS/tsx + Go + Python + Rust + Ruby + C + Java + C# (`RUSTCODEGRAPH_VALUE_REFS=0` disables). Shipped in **PR #895**
...
- **Tests:** `__tests__/value-reference-edges.test.ts` — same-file readers edged; surfaced in
  ...
  edged (tsx); `RUSTCODEGRAPH_VALUE_REFS=0` emits nothing.
...
Index the same repo twice (on vs `RUSTCODEGRAPH_VALUE_REFS=0`); node count **must be identical**
...
`rustcodegraph_search`/`callers`, *not* `impact`/`explore`, so it often doesn't query the
value-ref edges at all. ON was never worse than OFF. **So: value-refs does NOT reduce agent
reads — the win is blast-radius correctness** (impact API / RustCodeGraph Pro's verdict engine).
...
- **`RustCodeGraph.initSync(dir, opts)` ignores `opts`** — it takes only the path; the default
