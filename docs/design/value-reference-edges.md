...
**Status:** SHIPPED (default-on for TS/JS/tsx + Go + Python + Rust + Ruby + C + Java + C# + PHP + Scala + Kotlin + Swift + Dart + Pascal; `RUSTCODEGRAPH_VALUE_REFS=0` disables). The
...
flow straight into `getImpactRadius` / `rustcodegraph impact` and the impact trail in
`rustcodegraph_explore` / `rustcodegraph_node` — no agent-behaviour change required.
...
Method per repo: index the same tree twice (value-refs on vs `RUSTCODEGRAPH_VALUE_REFS=0`),
diff node/edge counts, spot-check precision, and measure `rustcodegraph impact` on a few
...
- **Impact API (the win):** `impact` ON vs OFF — `tablerIconProps` 1→170,
  `COLOR_PALETTE` 15→26, `CaptureUpdateAction` 61→86. This is what `rustcodegraph impact`
  and RustCodeGraph Pro's verdict engine consume via `getImpactRadius`.
- **Agent read-displacement: none — and that's expected.** On an indexed repo the agent
  answers impact questions in one rustcodegraph call (0 Read / 0 Grep in *both* arms), and it
  reaches for `rustcodegraph_search` / `callers`, **not** `impact`/`explore`, so it often
